...
func TestNewHandlers(t *testing.T) {
	t.Run("returns error when service is nil", func(t *testing.T) {
		...
		got, err := NewHandlers("app", nil, loggerMock)
	...
	t.Run("returns error when logger is nil", func(t *testing.T) {
		serviceMock := appmocks.NewMockServicesIF(t)

		got, err := NewHandlers("app", serviceMock, nil)
	...
	t.Run("returns handlers when params are valid", func(t *testing.T) {
		...
		got, err := NewHandlers("app", serviceMock, loggerMock)
		...
		require.True(t, ok)
		assert.Equal(t, "app", handler.name)
...
func TestHandlersGetName(t *testing.T) {
	handler := &handlers{name: "app"}

	assert.Equal(t, "app", handler.GetName())
}
...
func TestHandlersCreate(t *testing.T) {
	...
	t.Run("returns success when payload satisfies schema", func(t *testing.T) {
		...
		req := httptest.NewRequest(
			http.MethodPost,
			"/create",
			strings.NewReader(`{"name":"My App","nestedField":{"data":"Some data"}}`),
		)
		rec := httptest.NewRecorder()
		...
		assert.Equal(t, "hello from app handler", rec.Body.String())
	})
}
